...
async function main() {
  ...
  if (balance < 1000000000000000000n) {
    console.error(`[-] Insufficient GEN balance! Need at least 1.0 GEN to lock in escrow.`);
    console.log(`[-] Please send at least 1.5 GEN to: ${account.address}`);
    process.exit(1);
  }
  ...
  try {
    ...
    console.error(`[-] Failed to docket proposal:`, err);
    process.exit(1);
  }
}
